...
var PlaySound=function(url,vol,pitchVar,loop)
{
...
Game.Launch=function()
{
	...
	'<div class="listing">'+loc("CCX is a Cookie Clicker mod by zkayns.",'')+'</div>'+
	...
	var initMusic=function()
	{
		...
		if (AudioContext)
		{
			...
			Music.addTrack('preclick','C418','https://factual-cautious-gem.glitch.me/preclick.mp3');
			Music.addTrack('click','C418','https://factual-cautious-gem.glitch.me/click.mp3');
			Music.addTrack('grandmapocalypse','C418','https://factual-cautious-gem.glitch.me/grandmapocalypse.mp3');
			Music.addTrack('ascend','C418','https://factual-cautious-gem.glitch.me/ascend.mp3');
		...
		Game.loopT=0;
		Game.fps=30;
        if (!localStorage.nest) {
		Game.nest=1;
        } else {
        Game.nest=localStorage.getItem("nest")
        }
        if (!localStorage.forcenest) {
		Game.forcenest=0;
        } else {
        Game.forcenest=localStorage.getItem("forcenest")
        }
		...
		initMusic();
		l('versionNumber').innerHTML='v. '+Game.version+'x'+(!App?('<div id="httpsSwitch" style="cursor:pointer;display:inline-block;background:url(img/'+(Game.https?'lockOn':'lockOff')+'.png);width:16px;height:16px;position:relative;top:4px;left:0px;margin:0px -2px;"></div>'):'')+(Game.beta?' <span style="color:#ff0;">beta</span>':'');
		...
		Game.FileSave=function()
		{
			...
			saveAs(blob,filename+'.ccxsave');
		}
		Game.FileSaveLegacy=function()
		{
			if (App) return false;
			Game.prefs.showBackupWarning=0;
			var filename=Game.bakeryName.replace(/[^a-zA-Z0-9]+/g,'')+'Bakery';
		...
		Game.FileLoad=function(e)
		{
			...
			{
			    if (filename.split('.')[filename.split('.').length-1]=="ccxsave") {
                    Game.ImportSaveCode(atob(e.target.result));
                } else {
		...
		Game.slot2load=function(data,ignoreVersionIssues)
		{
			var str='';
			...
			else
			{
				...
				if (Game.useLocalStorage)
				{
					var local=localStorageGet(Game.SaveTo2);
					if (!local)//no localstorage save found? let's get the cookie one last time
					{
						if (document.cookie.indexOf(Game.SaveTo2)>=0)
						{
							str=unescape(document.cookie.split(Game.SaveTo2+'=')[1]);
							document.cookie=Game.SaveTo2+'=;expires=Thu, 01 Jan 1970 00:00:01 GMT;';
						}
						else return false;
					}
					else
				...
				{
					if (document.cookie.indexOf(Game.SaveTo2)>=0) str=unescape(document.cookie.split(Game.SaveTo2+'=')[1]);//get cookie here
					else return false;
				}
			}
			if (str!='')
			{
				...
				else
				{
					...
					if (Game.season!='' && Game.season==Game.baseSeason)
					{
						...
						else if (Game.season=='fools' && !Game.prefs.foolsoption) Game.Notify(loc("Business Day!"),loc("It's <b>Business season</b>!<br>Don't panic! Things are gonna be looking a little more corporate for a few days."),[17,6],60*3);
		...
		Game.shimmerTypes={
			...
			'golden':{
				...
				getTimeMod:function(me,m)
				{
					...
					if (Game.Has('Green yeast digestives')) m*=0.99; // heavenly yuck
		...
		Game.UpdateMenu=function()
		{
				...
				str+=
					'<div class="block" style="padding:0px;margin:8px 4px;">'+
						...
						'<div class="title">'+loc("General")+
							...
							//'<div class="listing"><a class="option smallFancyButton" '+Game.clickStr+'="slot2save();PlaySound(\'snd/tick.mp3\');">'+loc("Save (Slot 2)")+'</a><label>'+loc("Copy save to slot 2")+'</label></div>'+
							'<div class="listing"><a class="option smallFancyButton" '+Game.clickStr+'="Game.LoadSave();PlaySound(\'snd/tick.mp3\');">'+loc("Load")+'</a><label>'+loc("Loads save")+'</label></div>'+
							//'<div class="listing"><a class="option smallFancyButton" '+Game.clickStr+'="Game.slot2load();PlaySound(\'snd/tick.mp3\');">'+loc("Load (Slot 2)")+'</a><label>'+loc("Loads save from slot 2")+'</label></div>'+
							...
							(!App?('<div class="listing"><a class="option smallFancyButton" '+Game.clickStr+'="Game.FileSave();PlaySound(\'snd/tick.mp3\');">'+loc("Save to file")+'</a><a class="option smallFancyButton" '+Game.clickStr+'="Game.FileSaveLegacy();PlaySound(\'snd/tick.mp3\');">'+loc("Save to file (legacy)")+'</a><a class="option smallFancyButton" style="position:relative;"><input id="FileLoadInput" type="file" style="cursor:pointer;opacity:0;position:absolute;left:0px;top:0px;width:100%;height:100%;" onchange="Game.FileLoad(event);" '+Game.clickStr+'="PlaySound(\'snd/tick.mp3\');"/>'+loc("Load from file")+'</a><label>'+loc("Use this to keep backups on your computer")+'</label></div>'):'')+
					...
					'<div class="block" style="padding:0px;margin:8px 4px;">'+
						...
						'<div class="listing">'+
							...
							Game.WritePrefButton('foolsoption','foolsButton',loc("No fools")+ON,loc("No fools")+OFF,`Game.WriteSave();setTimeout('location.reload()', 1000);`)+'<label>('+loc("no fools")+')</label><br>'+ 
			...
			else if (Game.onMenu=='stats')
			{
				...
				'</div>'+
				'<br><div class="listing"><b>'+loc("Running version:")+'</b> '+Game.version+'x</div>'+
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			if (Game.TickerN%2==0 || loreProgress>14)
			{
				...
				if (Math.random()<0.75 || Game.cookiesEarned<10000)
				{
					...
					else
					{
                        ...
                        'News : This goober named his kid zkayns... bro.'
						]));
				...
				else
				{
					...
					'News : '+choose([
						'cookies found to '+choose(['increase lifespan','sensibly increase intelligence','reverse aging','decrease hair loss','prevent arthritis','cure blindness','cure ligma'])+' in '+choose(animals)+'!',
					...
					'News : "'+choose([
                        ...
                        'Yo, shawtys. Lil Gunk here. Cookies go hard, yuh.'
						])+'", reveals celebrity.',
					choose([
					...
					choose([
						...
						'News : '+Game.bakeryName+'-brand cookies "'+choose(['much less soggy','much tastier','relatively less crappy','marginally less awful','less toxic','possibly more edible','more fashionable','slightly nicer','trendier','arguably healthier','objectively better choice','slightly less terrible','decidedly cookier','a tad cheaper','more guapo','more skibidi'])+' than competitors", says consumer survey.',
                        'News : '+Game.bakeryName+'-brand cookies "going to LSU" this year',
						...
						'News : '+choose(['street','school','nursing home','stadium','new fast food chain','new planet','new disease','flesh-eating bacteria','deadly virus','new species of '+choose(animals),'new law','baby','programming language','JavaScript framework'])+' to be named after '+Game.bakeryName+', the world-famous cookie manufacturer.',
			...
			{
				list=[];
				if (Game.elderWrath==1) list.push(EN?choose([ //oh boy
				...
				if (Game.elderWrath==2) list.push(EN?choose([ //oh lawd
				...
				if (Game.elderWrath==3) list.push(EN?choose([ //phooey
		...
		Game.foolObjects={
			...
			'Bank':{name:'Investor',desc:'Business folks with a nose for profit, ready to finance your venture as long as there\'s money to be made.',icon:5},//melvin 
		...
		Game.jukebox={
			...
			setSound:function(id){
				...
				if (true) {PlaySound('snd/'+Game.jukebox.sounds[Game.jukebox.onSound]+'.mp3',1); } else {
                    PlaySound('snd/music/'+Game.jukebox.sounds[Game.jukebox.onSound].split('/')[1]+'.mp3',1);
                }
			},
			...
			pressMusicAuto:function(){
                //PlaySound('https://factual-cautious-gem.glitch.me/click.mp3')
...
//			str+='<div class="fancyText" style="font-variant:normal;letter-spacing:2px;padding:8px;font-size:18px;" id="jukeboxOnSound">&bull; '+Game.jukebox.sounds[Game.jukebox.onSound]+' &bull;</div>';
			...
			str+='<br></select><a class="option" onclick="Game.jukebox.setSound(Math.floor(Math.random()*Game.jukebox.sounds.length));">'+loc("Random")+'</a>';*/
			if (!App)
			{
				var data=Music?Music.tracks[Game.jukebox.tracks[Game.jukebox.onTrack]].audio:0;
				var dur=data?data.duration+1:0;
				str+='<div class="line"></div>';
				str+='<div class="fancyText" style="font-style:italic;letter-spacing:2px;padding:8px;font-size:15px;" id="jukeboxOnSound">&bull; '+cap(Game.jukebox.sounds[Game.jukebox.onSound])+' &bull;</div><br>';
				...
				*/
				str+='<a class="option fancyText" onclick="Game.jukebox.setSound(Game.jukebox.onSound-1);">&#xab;</a>';

                str+='<a class="option fancyText" onclick="Game.jukebox.setSound(Game.jukebox.onSound+1);">&#xbb;</a>';

                str+=/*'<a class="option fancyText prefButton'+(Game.jukebox.trackAuto?'':' off')+'" onclick="Game.jukebox.pressMusicAuto();" id="jukeboxMusicAuto">'+loc("Auto")+'</a>*/'<select id="jukeboxSoundSelect" onchange="Game.jukebox.setSound(parseInt(this.value));">';
		...
		new Game.Achievement('Third-party',loc("Use an <b>add-on</b>.")+'<q>Some find vanilla to be the most boring flavor.</q>',[16,5]);//if you're making a mod, add a Game.Win('Third-party') somewhere in there!
		...
		*/
				...
				{	
					if (Game.prefs.particles)
					{
						...
						else
						{
							...
							ctx.translate(x,y);
							if (Game.season=='easter' && Game.nest==1 || Game.forcenest==1)
							{
		...
		Game.OpenSesame=function()
		{
			...
			str+='<div class="line"></div>';
			str+='<a class="option neato" '+Game.clickStr+'="Game.cookies=parseInt(prompt(`New cookie #`))">Set cookies</a><br>';
			str+='<a class="option neato" '+Game.clickStr+'="Game.cookies*=2;Game.cookiesEarned*=2;">x2</a>';
			str+='<a class="option neato" '+Game.clickStr+'="Game.cookies/=2;Game.cookiesEarned/=2;">/2</a>';
			...
			str+='<a class="option neato" '+Game.clickStr+'="Game.DebugUpgradeCpS();">Debug upgrades CpS</a><br>';
			str+='<a class="option neato" '+Game.clickStr+'="Game.seed=Game.makeSeed();">Re-seed</a>';
			str+='<a class="option neato" '+Game.clickStr+'="Game.seed=prompt(`Seed`);">Set seed</a>';
			...
			str+='<div class="line"></div>';
			str+='<a class="option neato" '+Game.clickStr+'="eval(prompt(`JS`))">Run JS</a>';
			str+='</div>';
	...
	Game.Loop=function()
	{
        if (Game.prefs.foolsoption!==0&&Game.season=='fools') Game.season='';
        var randomthing = Math.floor(Math.random()*100)
        //Music.playTrack("click")
        //setInterval(PlaySound("music/click.mp3",1,0,1), 513000);
		...
		var time=Date.now();
        localStorage.setItem("music",Game.doMusic)
		localStorage.setItem("nest",Game.nest)
		localStorage.setItem("forcenest",Game.forcenest)		
		//latency compensator
