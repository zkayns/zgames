class TtyBlock {
    constructor(divName, numLines, rollOverLength, rollOverRemove) {
        ...
        this.lines = [];
        for (let i = 0; i < numLines; i++) this.lines.push("");
    ...
    */
    draw(dTime) {
        ...
        for (let i = 0; i < this.lines.length - 1; i++) outputString += `${this.lines[i]}<br/>`;
        outputString += lastLine.slice(0, Math.min(this.curPos, lastLine.length));
        if (this.cursorShown) outputString += "_";
        // rewirte for html guards
